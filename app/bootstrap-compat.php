...
add_action( 'after_switch_theme', 'uuups_blocked_switch_theme' );
add_action( 'load-customize.php', 'uuups_blocked_load_customize' );
add_action( 'template_redirect', 'uuups_blocked_preview' );
 ...
 * @return mixed
 */
function uuups_blocked_compat_message() {
	if ( version_compare( $GLOBALS['wp_version'], '4.9', '<' ) ) {

		return sprintf(
			/* translators: %1$s is supported WordPress version, and %2$s is WordPress version used. */
			esc_html__( 'Theme requires at least WordPress version %1$s. You are running version %2$s. Please upgrade and try again.', 'uuups-blocked' ),
			'4.9',
			$GLOBALS['wp_version']
		);
		...
		return sprintf(
			...
			esc_html__( 'Theme requires at least PHP version %1$s. You are running version %2$s. Please upgrade and try again.', 'uuups-blocked' ),
			'5.6',
			PHP_VERSION
		);
	}
 ...
 * @return void
 */
function uuups_blocked_switch_theme( $old_name ) {

	switch_theme( $old_name ? $old_name : WP_DEFAULT_THEME );

	unset( $_GET['activated'] );

	add_action( 'admin_notices', 'uuups_blocked_upgrade_notice' );
}
 ...
 * @return void
 */
function uuups_blocked_upgrade_notice() {
	printf( '<div class="error"><p>%s</p></div>', esc_html( uuups_blocked_compat_message() ) );
}
 ...
 * @return void
 */
function uuups_blocked_load_customize() {
	wp_die( esc_html( uuups_blocked_compat_message() ), '', array( 'back_link' => true ) );
}
 ...
 * @return void
 */
function uuups_blocked_preview() {
	if ( isset( $_GET['preview'] ) ) {
		wp_die( esc_html( uuups_blocked_compat_message() ) );
	}
}
